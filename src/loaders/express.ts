...
export default ({ app }: { app: express.Application }): void => {
  ...
  app.set('trust proxy', 2); //For heroku=1 elastic_benstalk=2(Because of loadbalancer webserver)

  // Enable Express Rate Limitter
  const limiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 15000, // Limit each IP to 100 requests per `window` (here, per 15 minutes)
