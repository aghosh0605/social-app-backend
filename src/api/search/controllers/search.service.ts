...
export const searchHomePage = async (
  ...
  try {
        ...
        if(req.body.searchData){ 
           let circleMatchingData= await circleCollection.find(
                {
                    circleName:{$regex:req.body.searchData,$options:'i'}
                }
            ).project({_id:0}).toArray();
            let usersMatchingData= await usersCollection.find(
                {
            ...
            let finalData={circleMatchingData,usersMatchingData}
              res
                .status(200)
                .json({ success: true, message:finalData });
            next(); 
        }
...
export const searchCirclePage = async (
    ...
    try {
          ...
          if(req.body.searchData){ 
             let circleMatchingData= await circleCollection.find({$or:
                 [ {circleName:{$regex:req.body.searchData,$options:'i'}},{
                ...
                res
                  .status(200)
                  .json({ success: true, message:circleMatchingData });
              next(); 
          }
